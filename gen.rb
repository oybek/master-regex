...
html = [
	...
	"",
	"function handleKeyup(){try{rgxPattern=new RegExp('^'+$('#user-input').val()+'$');}catch (e){return;}checkScore(rgxPattern,checkGoodStrs(rgxPattern),checkBadStrs(rgxPattern));}$(document).ready(function(){$('#user-input').keyup(handleKeyup);});</script><body><div class='main-block'><div class='block'><span>^</span><input type='text' id='user-input' autofocus/><span>$</span></div><hr noshade><div class='block'><span class='header'>Match:</span>",
	"",
